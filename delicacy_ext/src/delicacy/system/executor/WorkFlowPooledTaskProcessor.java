...
public class WorkFlowPooledTaskProcessor implements GenericProcessor {
    ...
    public String execute(String creteria, int employeeId) throws Exception {
        ...
        // 获得从前台传过来的任务池相关的数据
        Map<String, Object> result = (Map<String, Object>) JsonParser.parse(creteria);
        ...
        for(BaseSystemProcessPooledTask task : currentInstanceActivity.getTasks()) {
        	if(Objects.equals(task.getProcessPooledTaskId(), currentPooledTask.getProcessPooledTaskId()) && !Objects.equals(employeeId, task.getEmployeeId())) {
        ...
        // 获取用户增加的审批人姓名
        String additionalEmployeeName = BaseHelpUtils.getString(result.get("additionalEmployeeName"));
        ...
        int poolType = SystemProcessConstants.POOL_TYPE_0;
        if(null != processActivity) {
        	poolType = BaseHelpUtils.getIntValue(processActivity.getPoolType());
        }
        ...
        switch (approvalment) {
            case 0: // 同意
                ...
                if (poolType == SystemProcessConstants.POOL_TYPE_0) {
                    // 直接将当前节点设置为通过状态
            ...
            case 2://回退
                ...
                //因为串行任务池转交的问题，导致一个任务池节点可能存在一个人有多条记录的情况，所以这里针对员工ID去重
                if (ca.getTasks() != null && !ca.getTasks().isEmpty()) {
                	Set<Integer> taskOperatorIds = new HashSet<>();
                	for(BaseSystemProcessPooledTask task : ca.getTasks()) {
                		if(taskOperatorIds.contains(task.getEmployeeId())) {
                			continue;
                		}
                		taskOperatorIds.add(task.getEmployeeId());
                		task.setOperateTime(null);
                		task.setProcessComment(null);
                		instance.getTasks().add(task);
                	}
                }
                if (la.getTasks() != null && !la.getTasks().isEmpty()) {
                	Set<Integer> taskOperatorIds = new HashSet<>();
                	for(BaseSystemProcessPooledTask task : la.getTasks()) {
                		if(taskOperatorIds.contains(task.getEmployeeId())) {
                			continue;
                		}
                		taskOperatorIds.add(task.getEmployeeId());
            ...
            case 3://转交
                instance.setProcessStatus(SystemProcessConstants.PROCESS_STATUS_WORKING);
                if (poolType == SystemProcessConstants.POOL_TYPE_1) {
                	// 串行节点
        			//默认需要将转交人作为一个新任务池节点插入到已有的任务池中，但如果原任务池中已有此人，则不再插入
        			boolean needToAddTask = true;
        			BaseSystemProcessPooledTask additionalTask = new BaseSystemProcessPooledTask();
        			for(BaseSystemProcessPooledTask taskBean : instance.getTasks()) {
        				if(!Objects.equals(taskBean.getActivityId(), currentPooledTask.getActivityId())) {
        					continue;
        				}
        				if(Objects.equals(taskBean.getProcessPooledTaskId(), currentPooledTask.getProcessPooledTaskId())) {
        					taskBean.cloneCopy(additionalTask);
        					taskBean.setStatus(SystemProcessConstants.POOLED_TASK_STATUS_DONE);
        					taskBean.setProcessComment("转交给"+additionalEmployeeName+"审批");
        					taskBean.setOperateTime(new Date());
        				}else if(Objects.equals(taskBean.getEmployeeId(), additionalEmployee)) {
        					if(!(Objects.equals(taskBean.getStatus(), SystemProcessConstants.POOLED_TASK_STATUS_DONE) && null != taskBean.getProcessComment() && taskBean.getProcessComment().indexOf("转交") == 0)) {
        						needToAddTask = false;
        					}
        				}
        			}
        			if(needToAddTask) {
        				//需要往任务池节点中添加一个人，则流程不可能是完成状态
        				additionalTask.setMessageStatus(0);
        				additionalTask.setProcessComment(null);
        				additionalTask.setEmployeeId(additionalEmployee);
        				additionalTask.setOperateTime(null);
        				additionalTask.setProcessPooledTaskId(null);
        				instance.getTasks().add(additionalTask);
        			}else {
        				//如果不需要往该任务池节点中添加人员，则需要判断该节点是否已经全部人员审批完成，如果是，还要判断流程是否走完
        				// 是否跟该处理节点关联的所有任务池的任务都处理完了？
                        if (ProcessUtil.isAllPooledTaskPassed(instance, currentPooledTask)) {
                            // 激活下一个流程节点
                            completed = ProcessUtil.activeNextInstanceActivity(instance,
                            ...
                            ProcessUtil.passCurrentInstanceActivity(instance, currentInstanceActivity.getActivityId(),
                                    employeeId, currentInstanceActivity.getProcessComment(),
                                    SystemProcessConstants.ACTIVITY_STATUS_PASS, SystemProcessConstants.POOL_TYPE_1);
                            completed = ProcessUtil.findNextAvailableActivity(currentInstanceActivity.getNextActivityId(), instance.getActivities());
                        }
        			}
                }else {

        ...
        switch (approvalment) {
            case 0: // 通过
                ...
                if (null != lastActivity && Objects.equals(lastActivity.getStatus(), SystemProcessConstants.ACTIVITY_STATUS_RETURN_FIRST)) {
                    ...
                    completed = restartNewWorkflow(instance, pi, params, employeeId, definition, true);
                } else {
                    if (!completed) {
            ...
            case 2: // 回退
                ...
                for (int i = 0; i < instance.getActivities().size(); i++) {
                    ProcessInstanceActivity a = instance.getActivities().get(i);
                    if (Objects.equals(a.getActivityId(), currentInstanceActivity.getActivityId())) {
                        a.cloneCopy(ca);
                        a.setEmployeeId(employeeId);
                        a.setProcessComment(currentInstanceActivity.getProcessComment());
                        ...
                        nl.add(la);
                        /**
                         * 下面注释的这段代码，用于处理A转交给B，B会退给A后，A审批完，跳过B的情况
                         */
//                		BaseSystemProcessInstanceActivity backActivity = null;
//                		if(i != 0) {
//                			backActivity = instance.getActivities().get(i - 1);
//                		}
//                		//如果上一个节点是转交的，则本节点跳过
//                		if(null != backActivity && null != backActivity.getStatus() && Objects.equals(backActivity.getStatus(), SystemProcessConstants.ACTIVITY_STATUS_REASSIGN)) {
//                			ca = null;
//                			continue;
//                		}
            ...
            case 3: // 转交
                ...
                if (lastActivityIsReturnToDrafter(instance.getActivities(), currentInstanceActivity.getActivityId())) {
                    ...
                    resetProcessInstanceActivityId = true;
                    completed = restartNewWorkflow(instance, pi, params, employeeId, definition, false);
                } else {
                    completed = ProcessUtil.findNextAvailableActivity(currentInstanceActivity.getNextActivityId(), instance.getActivities());
                }
                break;
            case 4://	回退给发起人
                ...
                for (ProcessInstanceActivity activity : originalActivityList) {
                    ...
                    if (Objects.equals(activity.getActivityId(), currentInstanceActivity.getActivityId())) {
                        ...
                        activity.setProcessComment(currentInstanceActivity.getProcessComment());
    ...
    public boolean restartNewWorkflow(ProcessInstance instance, ProcessInformation pi, Map params, Integer employeeId, SystemProcessDefinition definition, boolean needToActiveFirstActivity) throws Exception {
        boolean completed = false;
        ...
        if (null != secondActivity.getProcessActivityId() && secondActivity.getProcessActivityId() == 0) {

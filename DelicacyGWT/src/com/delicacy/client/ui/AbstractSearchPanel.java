...
public abstract class AbstractSearchPanel extends DelicacyPanel {
    ...
    public Canvas generateHDataCanvas() {
        if (__detailCanvas != null && __detailCanvas.size() > 0) {
            SplitPane dataLayout = new SplitPane();
            dataLayout.setHeight100();
            dataLayout.setWidth100();
            dataLayout.setShowDetailToolStrip(false);
            dataLayout.setShowListToolStrip(false);
            dataLayout.setNavigationPane(resultGrid);
            resultGrid.setWidth("50%");
            if (__detailCanvas.size() == 1) {
                Canvas c1 = __detailCanvas.get(0);
                c1.setHeight100();
                c1.setWidth("50%");
                dataLayout.setListPane(c1);
                ...
                dataLayout.setListPane(sectionStack);
            }
            return dataLayout;
        } else {
    ...
    protected boolean __needViewPage = true;
	...
	public boolean is__needViewPage() {
		return __needViewPage;
	}

	public void set__needViewPage(boolean __needViewPage) {
		this.__needViewPage = __needViewPage;
	}

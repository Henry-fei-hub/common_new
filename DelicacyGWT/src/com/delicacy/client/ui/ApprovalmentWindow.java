 ...
 * @author guangxun
 */
public class ApprovalmentWindow extends Window implements HasHandlers {
    ...
    private boolean needPagination = false;//用来控制页面是否需要分页

    public ApprovalmentWindow() {
    ...
    public void initComponents() {
        ...
        __processId = ClientUtil.checkAndGetIntValue(sprocessId);
        ...
        if (activityType == 2) {
            ...
            viewPanel.viewRecord(new Record(), false);
...
//		mainLayout.addMember(workflowPanel);
    ...
    /**
     * 检测当前节点是否为活动状态，如果是，则提交数据；否则，提示用户节点不存在或状态异常
     *
     * @param submitType	审批类型【0:通过; 1:驳回; 2:回退; 3:转交; 4:回退给发起人】
     */
    public void checkAndSubmit(final int submitType) {
        submit(submitType);
    }

...
import com.delicacy.client.management.form.PermissionsDepartmentRolePanel;
import com.delicacy.client.management.form.PermissionsEmployeePanel;
import com.delicacy.client.management.form.PermissionsFunctionPanel;
import com.delicacy.client.management.form.PermissionsRolePanel;
 ...
 * @author lxf
 */
public class FunDepartmentRoleEmployeePanel extends VLayout {

	private static final Logger __logger = Logger.getLogger("");
	private PermissionsFunctionPanel functionDetailPanel;
	private PermissionsEmployeePanel employeeDetailPanel;
	private PermissionsRolePanel roleDetailPanel;
	private PermissionsDepartmentRolePanel departmentRoleDetailPanel;
	...
	public FunDepartmentRoleEmployeePanel() {
		...
		functionDetailPanel = new PermissionsFunctionPanel();
		...
		Label typeTitle = new Label("赋权对象选择");
		...
		saveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				String optType = "";
				if (permissionType == 1) {
					optType = "employeeData";
					condition.putAll(employeeDetailPanel.getValuesAsMap());
				}
				if (permissionType == 2) {
					optType = "roleData";
					condition.putAll(roleDetailPanel.getValuesAsMap());
				}
				if (permissionType == 3) {
					optType = "departmentRoleData";
					condition.putAll(departmentRoleDetailPanel.getValuesAsMap());
				}
				condition.put("operateEmployeeId", ClientUtil.getUserId());
				condition.put("optType", optType);
				DBDataSource.callOperation("EP_SaveRoleDepartmentEmployeeFunction", condition, new DSCallback() {
	...
	public void selectPermissionTypePanle(int permissionType) {
		...
		if (permissionType == 1) {
			...
			employeeDetailPanel = new PermissionsEmployeePanel();
		...
		if (permissionType == 2) {
			...
			roleDetailPanel = new PermissionsRolePanel();
		...
		if (permissionType == 3) {
			...
			deRoleButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					if (BaseHelpUtils.isNullOrEmpty(deRoleForm.getValue("departmentId"))) {
						departmentRoleDetailPanel.setDepartmentId(null);
					}else {
						departmentRoleDetailPanel.setDepartmentId(BaseHelpUtils.getIntValue(deRoleForm.getValue("departmentId")));
					}
					departmentRoleDetailPanel.findChildIdByDepartmenId();
			...
			departmentRoleDetailPanel = new PermissionsDepartmentRolePanel();
